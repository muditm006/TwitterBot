...
public class MarkovChain implements Iterator<String> {
    ...
    public void train(Iterator<String> sentence) {
        ...
        if (sentence.hasNext()) {
            String curWord = sentence.next();
            if (!curWord.isEmpty()) {
                startWords.record(curWord);
                while (sentence.hasNext()) {
                    String nextWord = sentence.next();
                    addBigram(curWord, nextWord);
                    curWord = nextWord;
                }
                addBigram(curWord, END_TOKEN);
            }
        }
    }
    ...
    public boolean hasNext() {
        return !curWord.equals(END_TOKEN);
    }
    ...
    public String next() {
        if (!hasNext()) {
            ...
            if (!chain.containsKey(curWord)) {
                ...
                ans = curWord;
                ProbabilityDistribution<String> table = chain.get(curWord);
                curWord = table.pick(ng);
            }
            return ans;
        }
    }

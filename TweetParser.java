...
public class TweetParser {
    ...
    static String extractColumn(String csvLine, int csvColumn) {
        ...
        try {
            String[] sentences = csvLine.split(",");
    ...
    static List<String> csvDataToTweets(BufferedReader br, int tweetColumn) {
        ...
        while (iterator.hasNext()) {
            String extracted = extractColumn(iterator.next(), tweetColumn);
            if (extracted != null) {
    ...
    static List<List<String>> parseAndCleanTweet(String tweet) {
        ...
        String sanitized = removeURLs(tweet);
        List<String> splitTweet = tweetSplit(sanitized);
        for (String sentence : splitTweet) {
            if (!parseAndCleanSentence(sentence).isEmpty()) {
                output.add(parseAndCleanSentence(sentence));
            }
        }
        return output;
    }
     ...
     * no sentences to be added. Do not return null.
     *
    ...
    public static List<List<String>> csvDataToTrainingData(
        ...
        List<List<String>> outputData = new LinkedList<List<String>>();
        List<String> tweets = csvDataToTweets(br, tweetColumn);
        for (String tweet : tweets) {
            List<List<String>> outputTweet = parseAndCleanTweet(tweet);
            if (!outputTweet.isEmpty()) {
                outputData.addAll(outputTweet);
            }
        }
        return outputData;
    }
